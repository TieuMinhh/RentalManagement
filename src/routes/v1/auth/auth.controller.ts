...
const logIn = asyncHandler(async (req, res) => {
  ...
  if (!user) return new NotFoundResponse(Messages.auth.notExitAccount).send(res);
  if (user.status === "lock") return new ForbiddenResponse(Messages.auth.lockedAccount).send(res);

  const isPasswordCorrect = await bcrypt.compare(password, user.password || "");
  if (!isPasswordCorrect) return new AuthFailureResponse(Messages.auth.wrongPassword).send(res);
...
//     const emailExists = await UserRepo.findOne({ email });
//     if (emailExists)
//       return new BadRequestResponse(Messages.auth.exitEmail).send(res);
//   }
...
//   if (user && file) {
//     const imageUrl = await uploadFileToCloudinary(file);
//     await UserRepo.update(user.id_account, { avatar: imageUrl });
...
//   await VerificationRepo.deleteToken(user.id_account, "reset_password");
